...
## One command
```
python collect.py <REG> [callsign] [aircraft_icao]
```
...
a flag in the row's Notes and carries on — it never crashes. A partial row is always written.
...
- **You (manual, always):** `OEW`, `Max Cargo`. Plus the **cabin-layout choice** when an
  operator has multiple configs (script defaults to latest; you confirm).
...
- `edigla_capture/`, `edigla_out/` — dumps for debugging.
...
- **Cabin:** `flyings.net/aircraft/{REG}/{type-slug}/` — **unreliable** (coverage/slug). Treated as the
  one field that stays operator/you-filled. Principle: **fetch all configs, default latest, user chooses.**
